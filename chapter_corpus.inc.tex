...
Though, the theoretical limitations of interpretability discussed in this chapter imply an \textit{upper limit} to understandability, not complete absence of it.
In order to achieve the maximal degree of interpretability for MDE methods, non-interpretable techniques need to be confined to some sub-tasks so that an interpretable pipeline can be built on top of the solutions to such sub-tasks.
The objective is to reduce to the minimum the responsibilities of black-box models in the pipeline.
This would make it easier to build trust in the resulting hybrid method and increase its transparency.
...
A loss function $\mathcal{L}$ is defined on image-depth map pairs.
...
Solving the learning problem means to identify a set of functions $\mathcal{H}$ called "hypothesis space" \cite{ML_book} that realize the mapping $\mathcal{I} \rightarrow \mathcal{Z}$ and pick up a function $f$ that minimizes the expected value of $\mathcal{L}$ over $\mathcal{D}$.
...
The process of choosing $f$ is called \textit{training} and consists in solving an optimization problem on $S$ that involves a simplified objective and some additional constraints that improve the solution (regularization terms).
In practice, the loss function used during training does not match the one to be globally minimized (called metric).
Also, many metrics can be used for assessing the performance of solutions to complex task like MDE.

Theorems have been proved about solving learning problems (see \cite{ML_book}) that express limitations of the learned solution and characterize learning problems in general.
Relevant to this thesis is the fact that one can simplify the learning problem by changing $\mathcal{D}$, $\mathcal{L}$ or $\mathcal{H}$ (the latter can be obtained also through regularization).
...
Empirical experiments must be further conducted for assessing this fact from a machine point of view, it is thus left to future works.
...
To mitigate this, in the sampling procedure, strips along the borders are ignored.
...
After having cropped and warped the patch, I experiment with radially blurring it\footnote{It is not a proper radial blur, but it produces a similar visual effect.}.
...
To the most blurred one $\mathbf{P}_{N}$ no cropping is applied, to $\mathbf{P}_{N-1}$ a center crop slightly smaller than the previous crop is applied, and so forth iteratively.
